...
## 완결 체크리스트 (게이트) — FT 경로 기준으로 갱신

- [ ] T1 FT 데이터 확보 (30ep 재사용 또는 50ep 보강) + 학습 완료
- [ ] T1 측정 10회 (SR/PR/지연/VRAM/실패유형)
- [ ] T2 FT 데이터 수집 + 학습 완료
- [ ] T2 측정 10회
- [ ] task별 대표 클립 각 1개 + 실패 대표 클립 1개
- [ ] 모델 요약 카드 (반 페이지) — ACT 대비 "사전학습 효과" 비교 포함
- [ ] 로그·클립 백업
...
| FT 후에도 성능이 ACT 이하일 가능성 | 정상적인 결과로 취급 — "사전학습이 항상 유리하진 않다"도 유효한 결론 |

## 결과 기록 (FT 완료 후 채울 것 — ACT 기록표(01_ACT.md)와 동일 형식 사용 예정)
