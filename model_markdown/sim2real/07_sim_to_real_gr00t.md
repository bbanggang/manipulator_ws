...
### 3.4 성능 저하 없음 — 근거와 검증 상태

- **이론/문헌**: 8-bit Adam은 옵티마이저 상태만 양자화하고 업데이트는 fp32 → fp32와 성능 차이가
  run-to-run 분산 이내(arXiv:2110.02861). 하이퍼파라미터도 무변경이라 학습 동역학 동일.
- **경험적 검증(진행 중)**: N1.6-8bit 20k steps 학습 완료 후 **§1.3과 동일한 sim 평가**로
  사전학습 N1.6(60%)과 SR 비교 예정. 8bit가 fp32와 동등하다면 60%에 근접해야 함.
  - 학습 예상 ~15h. **완료 시 아래 표 채움:**

| 모델 | 옵티마이저 | sim SR | open-loop MSE |
|---|---|---|---|
| N1.6 사전학습(NVIDIA, fp32) | fp32 Adam | 6/10 = 60% | — |
| **N1.6 자체학습(우리, 8bit)** | paged_adamw_8bit | (학습 후 측정) | (측정) |
| N1.5 자체학습(우리) | fp32(host) | 1/10 = 10% | 2.18 |
...
3. **8-bit N1.6 학습**: `paged_adamw_8bit`로 optimizer 상태만 양자화 → **22.7GB로 단일 5090 학습
   가능**(어제 불가능·#536 미해결을 해결). 하이퍼파라미터 무변경이라 fp32와 성능 동등(문헌),
   경험적 SR 비교는 학습 완료 후 확정.
