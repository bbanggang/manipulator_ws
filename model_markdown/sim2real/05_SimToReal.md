...
- **교정 시연 방법 (구현 확정 2026-07-21)**: 시작 자세를 **관절별로 무작위화**해 매 에피소드
  다른 "어긋난 상태"에서 시작 → 리더암으로 되돌아와 정렬 → 파지·배치 → 성공 종결.
  - 구현: `mdp/resets.py`에 `reset_joints_by_offset_per_joint()` 추가, `SIM_RECOVERY=1`로 토글
    (`setup/sim/patches/`). 범위: Rotation ±0.35rad(±20°), Pitch/Elbow ±0.25, 손목 ±0.15~0.20,
    **Jaw(그리퍼)는 0**(파지용이라 시작부터 흔들지 않음)
  - 수집: 정상 35ep(`record_sim.sh`) + 교정 15ep(`record_sim.sh recovery`) = 50ep, 70/30 비율
- **핵심**: 실패로 끝내지 말 것. "어긋남 → 교정 → 성공"의 **성공 종결 궤적**이어야 한다
- **sim의 이점**: 실기에서는 어긋난 자세를 매번 손으로 만들어야 해 번거롭고 재현성이 낮지만,
  sim은 **초기 자세를 시드로 무작위화**해 자동·대량 생성 가능. **범위가 리더암 복구 가능
  영역인지는 첫 교정 세션에서 조작감으로 검증** 후 확정
