# SmolVLA 모델 진행 보고서 — Zero-shot 불가 판정 → Few-episode FT (v1 T1 50%, v2 T1 30%)

> 작성일: 2026-07-13 | zero-shot 진단 → Few-episode FT(v1) → 50ep 보강 재학습(v2) T1 측정까지 완료
> 프로젝트: 단일 SO-ARM101 매니퓰레이터로 4개 모델(ACT / SmolVLA / π0 / GR00T N1.5)의
> 학습 및 zero-shot 성능을 비교한다. 본 문서는 두 번째 모델 **SmolVLA**의 zero-shot
> 시도 전 과정(왜 불가능한지 규명, §1~7), few-episode 파인튜닝 v1 결과(§8 T1 50%, §9 T2 부분진행),
> 그리고 **데이터 보강 v2 재학습·측정(§10 T1 30%)과 그로부터 규명한 epoch 교란변수**까지 정리한다.
...
| **SmolVLA-T1 FT (v1)** | 30ep로 학습·측정 (SR 50%, §8) | ✅ 완료 |
| **SmolVLA-T2 FT (v1)** | 30ep로 학습·측정 (엄격 SR 0%, 부분진행 6/8, §9) | ✅ 완료 |
| **T1 v2 (50ep 보강)** | 재수집·학습·측정 (SR 30%, epoch 교란 규명, §10) | ✅ 완료 |
| T1 v3 (epoch 통제) | 같은 50ep·steps 45k로 재학습 → 데이터 vs epoch 효과 분리 | ▶ 진행 중 |
| T2 v2 측정 | 학습 완료, 10회 측정 대기 (step 보정 필요성 검토) | 대기 |
| π0 | zero-shot **불가 판정**(카메라 3키·32차원 불일치, SmolVLA와 동일) → **LoRA FT로 전환** | 진행 중 |
